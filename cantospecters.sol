/**
             ...
             ('-.         .-') _  .-') _                        .-')     _ (`-.    ('-.             .-') _     ('-.  _  .-')    .-')    
            ( OO ).-.    ( OO ) )(  OO) )                      ( OO ).  ( (OO  ) _(  OO)           (  OO) )  _(  OO)( \( -O )  ( OO ).  
   .-----.  / . --. /,--./ ,--,' /     '._  .-'),-----.       (_)---\_)_.`     \(,------.   .-----./     '._(,------.,------. (_)---\_) 
  '  .--./  | \-.  \ |   \ |  |\ |'--...__)( OO'  .-.  '      /    _ |(__...--'' |  .---'  '  .--./|'--...__)|  .---'|   /`. '/    _ |  
  |  |('-..-'-'  |  ||    \|  | )'--.  .--'/   |  | |  |      \  :` `. |  /  | | |  |      |  |('-.'--.  .--'|  |    |  /  | |\  :` `.  
 /_) |OO  )\| |_.'  ||  .     |/    |  |   \_) |  |\|  |       '..`''.)|  |_.' |(|  '--.  /_) |OO  )  |  |  (|  '--. |  |_.' | '..`''.) 
    ...
    ClampedRandomizer
{
    ...
    address  public royaltyAddress = ROYALTIESSET;
    ...
    uint256 public maxPerPerson = 30;
    uint256 public price = 100 ether;


    uint256 public royalty = 680;
    // COLLECTED FESS
    struct DevFee {
        uint256 percent;
        uint256 amount;
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("CantoSpecters", "SPECTER") ClampedRandomizer(maxSupply) {     
    ...
    function setRoyalty(uint16 _royalty) external onlyOwner {
        ...
        require(_royalty <= 680, "Royalty must be greater than or equal to 7,5%" );
        royalty = _royalty;
    }
...
//Special thanks to CL for Inspiration!  
