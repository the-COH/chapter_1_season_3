...
export class SplitsClient extends BaseClient {
  ...
  */
  ...
  async getTopSplits({ lastId }: { lastId: string }): Promise<{
    splits: Split[]
  }> {
    const response = await this._makeGqlRequest<{
      splits: GqlSplit[]
    }>(TOP_SPLITS_QUERY, { lastId: lastId.toLowerCase() })
  ...
  async getTopRecipients({ lastId }: { lastId: string }): Promise<{
    recipients: SplitRecipient[]
  }> {
    const response = await this._makeGqlRequest<{
      recipients: GqlRecipient[]
    }>(TOP_RECIPIENTS_QUERY, { lastId: lastId.toLowerCase() })

    const [recipients] = await Promise.all([
      Promise.all(
        response.recipients.map(
          async (gqlRecipient) => await this._formatRecipient(gqlRecipient),
        ),
      ),
    ])
  ...
  async getSplitEarnings({
    ...
    const erc20Tokens = this._splitMain.provider
      ? await fetchERC20TransferredTokens(splitId)
      : []
  ...
  */
  ...
  private async _formatRecipient(
    gqlRecipient: GqlRecipient,
  ): Promise<SplitRecipient> {
    const recipient = formatRecipient(gqlRecipient)
